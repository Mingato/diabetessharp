...
async function seed() {
  ...
  await query(
    `INSERT INTO cognitive_progress ("userId", date, "cognitiveScore", "memoryScore", "processingSpeed", "attentionScore", "dailyStreak", "phaseNumber")
     VALUES ($1, $2, 55, 55, 55, 55, 3, 1)
     ON CONFLICT ("userId", date) DO UPDATE SET "cognitiveScore" = 55, "dailyStreak" = 3`,
    [userId, today]
  );
