...
export const affiliateRouter = router({
    getDashboard: protectedProcedure.query(async ({ ctx }) => {
        const clicks = await query(`SELECT COUNT(*) as count FROM affiliate_clicks ac
       JOIN affiliates a ON a.id = ac."affiliateId" WHERE a.email = $1`, [ctx.user.email]);
        const conv = await query(`SELECT COUNT(*) as count FROM affiliate_commissions ac
       JOIN affiliates a ON a.id = ac."affiliateId" WHERE a.email = $1`, [ctx.user.email]);
        const comm = await query(`SELECT COALESCE(SUM("commissionAmount"), 0) as sum FROM affiliate_commissions ac
       JOIN affiliates a ON a.id = ac."affiliateId" WHERE a.email = $1`, [ctx.user.email]);
        const payouts = await query(`SELECT amount, status, created_at FROM affiliate_payouts ap
       JOIN affiliates a ON a.id = ap."affiliateId" WHERE a.email = $1 ORDER BY created_at DESC LIMIT 10`, [ctx.user.email]);
        const aff = await query(`SELECT code FROM affiliates WHERE email = $1`, [
            ctx.user.email,
        ]);
        return {
    ...
    trackClick: publicProcedure
        ...
        const aff = await query(`SELECT id FROM affiliates WHERE code = $1 AND status = 'active'`, [
            input.ref,
        ]);
        if (aff.rows[0]) {
            await query(`INSERT INTO affiliate_clicks ("affiliateId", source) VALUES ($1, $2)`, [aff.rows[0].id, input.source]);
        }
        return { success: true };
    }),
});
