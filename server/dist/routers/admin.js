...
export const adminRouter = router({
    getOrders: adminProcedure.query(async () => {
        const r = await query(`SELECT id, "firstName", "lastName", email, "cognitiveProfile", status, "amountPaid", "cognitiveRiskScore", created_at
       FROM funnel_orders ORDER BY created_at DESC LIMIT 200`);
        return { orders: r.rows };
    }),
    getAffiliateStats: adminProcedure.query(async () => {
        const aff = await query(`SELECT id, name, email, code, tier, "totalCommissionEarned", status FROM affiliates`);
        const comm = await query(`SELECT COALESCE(SUM("commissionAmount"), 0) as sum FROM affiliate_commissions`);
        const top = await query(`SELECT a.name, a.code, COUNT(ac.id) as conversions, SUM(ac."commissionAmount") as total
    ...
    createTestOrder: adminProcedure
        ...
        const order = await query(`INSERT INTO funnel_orders ("firstName", "lastName", email, status, "amountPaid") VALUES ($1, $2, $3, 'pending', 2999) RETURNING id`, [input.firstName, input.lastName, input.email]);
    ...
    getSupportTickets: adminProcedure.query(async () => {
        const r = await query(`SELECT id, name, email, category, subject, message, "orderId", status, created_at
    ...
    createAffiliate: adminProcedure
        ...
        await query(`INSERT INTO users (email, password_hash, role) VALUES ($1, $2, 'affiliate') ON CONFLICT (email) DO NOTHING`, [input.email, hash]);
        await query(`INSERT INTO affiliates (name, email, code, status) VALUES ($1, $2, $3, 'active')
         ON CONFLICT (email) DO NOTHING`, [input.name, input.email, input.code]);
        return { success: true };
    }),
});
