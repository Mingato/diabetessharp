...
function SidebarNavLink({ path, label, Icon }: { path: string; label: string; Icon: React.ComponentType }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium min-h-[44px] w-full text-left border border-transparent transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-surface-elevated)] ${
          isActive
  ...
  return (
    <div className="flex min-h-screen min-h-[100dvh] h-screen overflow-hidden app-bg">
      {/* Desktop sidebar — glassmorphism, 8px grid spacing */}
        ...
        <div className="p-6 border-b border-[var(--color-border-subtle)] shrink-0">
          <Link to="/app/dashboard" className="flex items-center gap-3 group">
            <div className="w-11 h-11 rounded-xl bg-[var(--color-card)] border border-[var(--color-border-subtle)] flex items-center justify-center overflow-hidden shadow-[var(--shadow-elevation-low)] transition-[transform,box-shadow] duration-200 ease-[var(--ease-spring)] group-hover:shadow-[var(--shadow-glow)] group-hover:scale-[1.02]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <main
          ref={mainRef}
          className="flex-1 overflow-y-auto overflow-x-hidden app-main p-4 sm:p-5 pb-36 md:p-8 lg:pb-0 max-w-4xl mx-auto w-full relative"
          role="main"
        >
          <Outlet />

          {/* Floating AI Doctor — primary CTA with glow */}
          <button
            type="button"
            onClick={() => navigate("/app/dr-marcus")}
            className="fixed right-4 bottom-[88px] sm:right-8 sm:bottom-[96px] z-40 inline-flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-[var(--gradient-accent)] text-white text-sm font-bold shadow-[0_8px_32px_var(--color-accent-glow)] hover:shadow-[0_12px_40px_var(--color-accent-glow)] hover:-translate-y-0.5 active:scale-[0.98] border-0 transition-all duration-200 ease-[var(--ease-spring)] float-cta-glow"
          >
